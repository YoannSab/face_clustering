...
def find_faces(image_path:str):
    try :
        ...
        if image.shape[0] > 500 or image.shape[1] > 500:
            image = cv2.resize(image, (int(image.shape[1]*(400/image.shape[0])) , 
                                       int(image.shape[0]* (400/image.shape[0]))))
...
def get_image_by_face():
    try :
        ...
        average_length = sum([len(p) for p in file_paths]) / len(file_paths)
        #cut file paths in chuck of 100 to avoid exiftool error
        chunk_length = 150
        all_results = ""
